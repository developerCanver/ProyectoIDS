...
void IMAP_InitCmds(IMAPConfig *config)
{
    ...
    config->cmds = (IMAPToken *)_dpd.snortAlloc(CMD_LAST + 1, sizeof(IMAPToken), PP_IMAP, 1);
    if (config->cmds == NULL)
    {
    ...
    config->cmd_search = (IMAPSearch *)_dpd.snortAlloc(CMD_LAST, sizeof(IMAPSearch), PP_IMAP, 1);
...
static IMAP * IMAP_GetNewSession(SFSnortPacket *p, tSfPolicyId policy_id)
{
    ...
    ssn = (IMAP *)_dpd.snortAlloc(1, sizeof(IMAP), PP_IMAP, 0);
    if (ssn == NULL)
    {
	...
	_dpd.snortFree(ssn, sizeof(*ssn), PP_IMAP, 0);
        return NULL;
    }
...
static void IMAP_SessionFree(void *session_data)
{
	...
	_dpd.snortFree(imap->mime_ssn.decode_state, sizeof(Email_DecodeState), PP_IMAP, 0);
    }

    if(imap->mime_ssn.log_state != NULL)
    {
        mempool_free(imap_mempool, imap->mime_ssn.log_state->log_hdrs_bkt);
	_dpd.snortFree(imap->mime_ssn.log_state, sizeof(MAIL_LogState), PP_IMAP, 0);
    }
    if ( ssl_cb )
        ssl_cb->session_free(imap->flow_id);

    _dpd.snortFree(imap, sizeof(*imap), PP_IMAP, 0);
...
void IMAP_FreeConfig(IMAPConfig *config)
{
    ...
    if (config->cmds != NULL)
    {
	    ...
	    _dpd.snortFree(tmp->name, sizeof(*(tmp->name)), PP_IMAP, 1);

	_dpd.snortFree(config->cmds, sizeof(*(config->cmds)), PP_IMAP, 1);
    }
    ...
    if (config->cmd_search != NULL)
	_dpd.snortFree(config->cmd_search, sizeof(*(config->cmd_search)), PP_IMAP, 1);

    _dpd.snortFree(config, sizeof(*config), PP_IMAP, 1);
}
