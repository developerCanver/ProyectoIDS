...
void POP_InitCmds(POPConfig *config)
{
    ...
    config->cmds = (POPToken *)_dpd.snortAlloc(CMD_LAST + 1, sizeof(POPToken), PP_POP, 1);
    if (config->cmds == NULL)
    {
    ...
    config->cmd_search = (POPSearch *)_dpd.snortAlloc(CMD_LAST, sizeof(POPSearch), PP_POP, 1);
...
static POP * POP_GetNewSession(SFSnortPacket *p, tSfPolicyId policy_id)
{
    ...
    ssn = (POP *)_dpd.snortAlloc(1, sizeof(POP), PP_POP, 0);
    if (ssn == NULL)
    {
    ...
    if (( ret = _dpd.fileAPI->set_log_buffers(&(pop_ssn->mime_ssn.log_state), &(pPolicyConfig->log_config),pop_mempool, p->stream_session, PP_POP)) < 0)
    {
        ...
        _dpd.snortFree(ssn, sizeof(*ssn), PP_POP, 0);
        return NULL;
    }
...
static void POP_SessionFree(void *session_data)
{
    ...
    if(pop->mime_ssn.decode_state != NULL)
    {
        ...
        _dpd.snortFree(pop->mime_ssn.decode_state, sizeof(Email_DecodeState), PP_POP, 0);
    }

    if(pop->mime_ssn.log_state != NULL)
    {
        mempool_free(pop_mempool, pop->mime_ssn.log_state->log_hdrs_bkt);
        _dpd.snortFree(pop->mime_ssn.log_state, sizeof(MAIL_LogState), PP_POP, 0);
    ...
    _dpd.snortFree(pop, sizeof(*pop), PP_POP, 0);
...
void POP_FreeConfig(POPConfig *config)
{
    ...
    if (config->cmds != NULL)
    {
        ...
        for (; tmp->name != NULL; tmp++)
            _dpd.snortFree(tmp->name, sizeof(*(tmp->name)), PP_POP, 1);

        _dpd.snortFree(config->cmds, sizeof(*(config->cmds)), PP_POP, 1);
    }
    ...
    if (config->cmd_search != NULL)
        _dpd.snortFree(config->cmd_search, sizeof(*(config->cmd_search)), PP_POP, 1);

    _dpd.snortFree(config, sizeof(*config), PP_POP, 1);
}
