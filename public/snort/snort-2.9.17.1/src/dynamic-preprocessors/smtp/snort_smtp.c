...
void SMTP_InitCmds(SMTPConfig *config)
{
    ...
    config->cmds = (SMTPToken *)_dpd.snortAlloc(CMD_LAST + 1, sizeof(SMTPToken), PP_SMTP, 1);
    if (config->cmds == NULL)
    {
    ...
    config->cmd_search = (SMTPSearch *)_dpd.snortAlloc(CMD_LAST, sizeof(SMTPSearch), PP_SMTP, 1);
...
static SMTP * SMTP_GetNewSession(SFSnortPacket *p, tSfPolicyId policy_id)
{
    ...
    ssn = (SMTP *)_dpd.snortAlloc(1, sizeof(SMTP), PP_SMTP, 0);
    if (ssn == NULL)
    {
	...
	_dpd.snortFree(ssn, sizeof(*ssn), PP_SMTP, 0);
        return NULL;
    }
...
static void SMTP_SessionFree(void *session_data)
{
	...
	_dpd.snortFree(smtp->mime_ssn.decode_state, sizeof(Email_DecodeState), PP_SMTP, 0);
    }

    if(smtp->mime_ssn.log_state != NULL)
    {
        mempool_free(smtp_mempool, smtp->mime_ssn.log_state->log_hdrs_bkt);
	_dpd.snortFree(smtp->mime_ssn.log_state, sizeof(MAIL_LogState), PP_SMTP, 0);
    }

    if(smtp->auth_name != NULL)
    {
	_dpd.snortFree(smtp->auth_name, sizeof(*(smtp->auth_name)), PP_SMTP, 0);
    }
    if ( ssl_cb )
        ssl_cb->session_free(smtp->flow_id);

    _dpd.snortFree(smtp, sizeof(*smtp), PP_SMTP, 0);
...
void SMTP_FreeConfig(SMTPConfig *config)
{
    ...
    if (config->cmds != NULL)
    {
	    ...
	    _dpd.snortFree(tmp->name, sizeof(*(tmp->name)), PP_SMTP, 1);
    ...
    _dpd.snortFree(config, sizeof(*config), PP_SMTP, 1);
}
...
static bool SMTP_IsAuthChanged(const uint8_t *start_ptr, const uint8_t *end_ptr)
{
    ...
    if (smtp_ssn->auth_name)
    {
        if (smtp_ssn->auth_name->length != length)
          ...
          smtp_ssn->auth_name = _dpd.snortAlloc(1, sizeof(*(smtp_ssn->auth_name)), PP_SMTP, 0);
...
void SnortSMTP(SFSnortPacket *p)
{
    ...
    if( &smtp_no_session == smtp_ssn )
    {
	     ...
	     _dpd.snortFree(smtp_ssn->mime_ssn.decode_state, sizeof(Email_DecodeState), PP_SMTP, 0);
	     ...
	     _dpd.snortFree(smtp_ssn->mime_ssn.log_state, sizeof(MAIL_LogState), PP_SMTP, 0);
             smtp_ssn->mime_ssn.log_state = NULL;
         }

         if(smtp_ssn->auth_name != NULL)
         {
	     _dpd.snortFree(smtp_ssn->auth_name, sizeof(*(smtp_ssn->auth_name)), PP_SMTP, 0);
